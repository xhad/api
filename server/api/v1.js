...
*/
router.get("/orderbook/:id", function(req, res){
...
endpoint:  GET /ticker/:id
:id  currency_pair_id (number)
body json: none
query params: none
*/
router.get("/ticker/:id", function(req, res){
  //query trades table for latest trade
  Trade.where({currency_pair:req.params.currency_pair})
  .query('orderBy', 'created_at', 'desc')
  .fetch({columns:['currency_pair_id','price','size','created_at']})
  .then(function(trade){
    if(!trade) return;
    return {
      currency_pair_id: trade.get('currency_pair_id'),
...
router.get('*', function(req, res) {
   res.status = 404;
   res.json({
      status: 'Welcome to the Chaintrader API',
      contact: 'Contact xhad for help or see the documentation at http://api.chaintrader.com',
